...
static RECENT_USERS: OnceCell<Mutex<VecDeque<usize>>> = OnceCell::new();

#[derive(Parser)]
pub struct SocialPrefetch {
...
#[async_trait]
impl util::Backend for SocialPrefetch {
    async fn prepare(&self) {
        ...
        let social_file =
            format!("/users/{username}/MuCache_Extended/experiments/social/socfb/socfb-analysis.txt");
        ...
        SAMPLE_POST
            ...
            .unwrap();
        RECENT_USERS.set(Mutex::new(VecDeque::with_capacity(100))).unwrap();
    }
...
pub fn similar_user() -> usize {
    let mut recent_user_guard = RECENT_USERS.get().unwrap().lock().unwrap();
    if !recent_user_guard.is_empty() && rand::thread_rng().gen_bool(0.75) {
        let user_idx = recent_user_guard.pop_back().unwrap().clone();
        println!("Getting nearby user {}", user_idx);
        user_idx
    } else {
        let user_idx = random_user();
        println!("Getting random user {} and adding neighbors to queue", user_idx );
        if user_idx > 0 {
            println!("Adding neighbor {} to queue", user_idx - 1 );
            recent_user_guard.push_front(user_idx - 1);
        }
        if user_idx < MAX_USER.get().unwrap() - 1 {
            println!("Adding neighbor {} to queue", user_idx + 1 );
            recent_user_guard.push_front(user_idx + 1);
        }
        user_idx
...
pub fn read_similar_user_timeline() -> ReadTimeline {
    // Add prefetching logic here for random_user
    let user_idx = similar_user();
    let user_str = "User".to_owned() + &user_idx.to_string();
...
pub async fn service_read_usertimeline(_req: Request<Body>) -> Result<Response<Body>, Infallible> {
    // info!("usertimeline");
    let c = read_similar_user_timeline();
    let req = serde_json::to_string(&c).unwrap();
    let ip: &String;
    if *SHARD.get().unwrap() == 1 {
...
pub async fn service_social(_req: Request<Body>) -> Result<Response<Body>, Infallible> {
    let read_home_timeline_ratio = 50;
    let read_user_timeline_ratio = 40;
