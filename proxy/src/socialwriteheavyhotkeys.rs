...
#[derive(Parser)]
pub struct SocialWriteHeavyHotKeys {
...
#[async_trait]
impl util::Backend for SocialWriteHeavyHotKeys {
...
pub fn random_user() -> usize {
    ...
    // Very high likelihood of getting first 10
    if coin < 50 {
        rand::random::<usize>() % 10 + 1
    } else {
...
pub async fn service_social(_req: Request<Body>) -> Result<Response<Body>, Infallible> {
    let read_home_timeline_ratio = 20;
    let read_user_timeline_ratio = 20;
    let compose_post_ratio = 60;
    assert_eq!(
