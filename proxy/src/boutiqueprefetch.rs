...
static RECENT_PRODUCTS: OnceCell<Mutex<VecDeque<String>>> = OnceCell::new();

#[derive(Parser)]
pub struct BoutiquePrefetch {
...
#[async_trait]
impl util::Backend for BoutiquePrefetch {
    async fn prepare(&self) {
        ...
        RECENT_PRODUCTS.set(Mutex::new(VecDeque::with_capacity(100))).unwrap();
    }
...
pub fn similar_product_str() -> String {

    let mut recent_products_guard = RECENT_PRODUCTS.get().unwrap().lock().unwrap();
    if !recent_products_guard.is_empty() && rand::thread_rng().gen_bool(0.75) {
        let product_idx = recent_products_guard.pop_back().unwrap().clone();
        //println!("Getting nearby product {}", product_idx);
        "p".to_owned() + &product_idx
    } else {
        let product_idx = random_product();
        //println!("Getting random product {} and adding neighbors to queue", &product_idx.to_string() );
        if product_idx > 0 {
            //println!("Adding neighbor {} to queue", (product_idx - 1).to_string() );
            recent_products_guard.push_front((product_idx - 1).to_string());
        }
        if product_idx < TOTAL_PRODUCTS.get().unwrap() - 1 {
            //println!("Adding neighbor {} to queue", (product_idx + 1).to_string() );
            recent_products_guard.push_front((product_idx + 1).to_string());
        }
        "p".to_owned() + &product_idx.to_string()
...
pub fn browse_product() -> BrowseProduct {
    let product_str = similar_product_str();
    println!("Browsing product {}", product_str );
