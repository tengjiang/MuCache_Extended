...
static RECENT_MOVIES: OnceCell<Mutex<VecDeque<usize>>> = OnceCell::new();

#[derive(Parser)]
pub struct MoviePrefetch {
...
#[async_trait]
impl util::Backend for MoviePrefetch {
    async fn prepare(&self) {
        ...
        RECENT_MOVIES.set(Mutex::new(VecDeque::with_capacity(100))).unwrap();
    }
...
pub fn similar_movie() -> usize {
    let mut recent_movies_guard = RECENT_MOVIES.get().unwrap().lock().unwrap();
    if !recent_movies_guard.is_empty() && rand::thread_rng().gen_bool(0.75) {
        let movie_idx = recent_movies_guard.pop_back().unwrap().clone();
        println!("Getting nearby movie {}", movie_idx);
        movie_idx
    } else {
        let movie_idx = random_movie();
        println!("Getting random movie {} and adding neighbors to queue", &movie_idx.to_string() );
        if movie_idx > 0 {
            println!("Adding neighbor {} to queue", (movie_idx - 1).to_string() );
            recent_movies_guard.push_front(movie_idx - 1);
        }
        if movie_idx < TOTAL_MOVIES.get().unwrap() - 1 {
            println!("Adding neighbor {} to queue", (movie_idx + 1).to_string() );
            recent_movies_guard.push_front(movie_idx + 1);
        }
...
pub fn page() -> Page {
    // View similar movies via index HERE
    let movie_idx = similar_movie();
