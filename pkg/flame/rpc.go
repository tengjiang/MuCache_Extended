...
// `shared_memory_queue_capacity_shift` is bumped from 8 → 11 on tcs_api,
...
// cursor has a full queue worth of margin before wrapping. With the old
// value of 512, sustained concurrent sends hit SIGBUS inside
// flame_client_send because the cursor wraps past the still-in-use shm
// slot range; bumping to 4096 gives a full queue of headroom.
//
// The local start script's daemon --window-size must match (see
// scripts/local/start_chain.sh).
	...
	go func() {
		for {
			...
			go func() {
				...
				// Reuse a pooled RpcMsgSize-sized scratch instead of allocating per response.
